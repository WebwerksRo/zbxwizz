...
function dataTable(id) {
    function rowObj(row) {
        ...
        let obj = {
            ...
            el: $row,
            get cells(){return this.el.children(":not(:first-child)").toArray()},
            get selected(){ console.log(this.el); return this.el.hasClass("selected");}
        };
    ...
    const dt = {
        ...
        load_data: function (data,type="csv") {
            ...
            for(let i=0;i<numCols;i++) {
                ...
                $("<th>").data("fldname",fld).attr("data-col",col).append($("#transformTpl").clone().attr("id",null)).appendTo(transformRow).find("textarea[name=xpresion]").autocomplete({source: autocompleteData.options});
            ...
            for(let rowIdx=0;rowIdx<data.records.length;rowIdx++) {
                ...
                for(let colIdx=0;colIdx<numCols;colIdx++) {
                    ...
                    let cell = $("<td>")
                        ...
                        .on("dblclick",(event)=>$(event.target).attr('contenteditable',true))
...
var autocompleteData = {
    get options() {
        ...
        Object.keys(localStorage).filter(key=>key.indexOf("transfo_")!==-1).forEach(key=>{
            opts.push(localStorage.getItem(key));
        });
        return opts;
    }
}
...
function minimize_transform(event) {
    console.log(event);
    let slf = event.target;
    if($(event.relatedTarget).parents(".transform")[0]==$(slf).parents(".transform")[0] || event.relatedTarget===null) {
        return;
    } 

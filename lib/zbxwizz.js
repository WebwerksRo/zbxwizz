...
function transform_data(event, apply = false) {
    ...
    if(event.keyCode) return event.target.blur();
    let src = event.target;
    let el = src.form.xpression;
    let col = $(el).parents("th").attr("data-col");
    let expr = el.value;
...
function save_structure() {
    ...
    let name = prompt("Input table structure name");
    localStorage.setItem("tbl_struct_"+name,json(worksheets.get_active_sheet().columnFields));
...
function manage_struct() {
    ...
    show_modal({
        body:`
        Saved table structures<br>
        <form>
        <select name="struct"><option></option>${tmp.join()}</select><br>

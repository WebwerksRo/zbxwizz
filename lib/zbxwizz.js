...
function import_zbx(reqTpl, form) {
    ...
    let params;
    try {
        params = JSON.parse(eval("`" + reqTpl + "`"));
        console.log(params);
    } catch (e) {
        return show_modal({
            body: 'Invalid request:<pre class="pre" style="overflow: scroll">'+reqTpl+'</pre>'
        })
    }
...
function push(template, form) {
    ...
    show_modal({
        body: $("<div>").append("<p>Pushing to Zabbix can lead to damage if the request is not properly configured. Please confirm you have reviewed the request and you are sure you want to perform the operation</p>")
    ...
    function exec_push(template, form) {
        ...
        Promise.all(ps).finally(()=>{
            overlay.hide();
            if(err) {
                show_modal({body: "Some error occured: "+err.message})
            }
        });
    }
...
function pull(template, form) {
    if (form.postprocess.value === "update") {
        let ps = [];
        let rows = get_active_sheet().rows.filter(row => row.isSelected);
                    ...
                    let p = zbx.get(form.resource.value, tmp)
                        .then((resp) => {
                            if (resp.result && resp.result.length) {
                                row.data[form.label.value] = resp.result.length==1 ? resp.result[0] : resp.result ;
                            }
...
function transform_col(colId,expr) {
    worksheets.get_active_sheet()[typeof colId==="number"?"get_col_by_num":"get_col_by_fld"](colId)
...
function transform_data(event, apply = false) {
    ...
    if (!apply) {
        let dt = worksheets.get_active_sheet();
        let cell = dt.get_col_by_num(col).filter(cell=>!cell.row.isHidden).shift();
...
function autosave(stop=false) {
    ...
    Object.keys(worksheets.sheets).forEach((name)=>{
         ...
         * @type {DataTable}
         */
        let tmp = worksheets.sheets[name];
        tmp.save();
    });
    if(!stop) 

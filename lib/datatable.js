...
class DataTable {
    ...
    <th>
    <div class="d-flex">
    <div><button class="badge badge-light" onclick="move_col_left(this)">&lt;</button></div>
    <div class="flex-grow-1"><button class="colno badge badge-secondary w-100"></button></div>
    <div><button class="badge badge-light" onclick="move_col_right(this)">&gt;</button></div>
</div>
</th>
    `
  ...
  <input type="checkbox">
    ...
    load_data(fields,records,dataLabel="csv") {
        ...
        function hide_col() {
            self.toggle_compress_coll($(this).parents("th").data("col"));
        }
        ...
        return new Promise(((resolve, reject) => {
            ...
            for(let i=0;i<numCols;i++) {
                ...
                $(this.#colCellTpl).appendTo(colsRow).attr("data-col",i)
                    .find(".colno").text(i).on("click",hide_col);
            ...
            this.fieldsOrder = {};
            fields.forEach((fld,idx)=>this.fieldsOrder[fld]=idx);

            function toggle_all_visible(event,dt) {
                let state = event.currentTarget.checked;

                dt.rows.filter(row=>!row.is_hidden).forEach(row=>row.select(state));
            }

            let self = this;
            // render data
            $('<input type="checkbox" id="selectAll">').appendTo(ctrlCell).on('click',(event)=>toggle_all_visible(event,self));
            this.#tbody.empty();
            // this.add_row(fields,records[0],0);
            // resolve();
            // return;
            // records.forEach((record,rowIdx)=>{
            //     this.add_row(fields,record,rowIdx) = records;
            // });
            records.forEach((record,rowIdx)=>{
...
function get_unique(src){
    let sel = $(src).empty();    
    worksheets.get_active_sheet().get_col_by_num(sel.parents("th").data().col)  // get cols

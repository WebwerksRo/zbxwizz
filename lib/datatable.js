...
<th>
    <div class="colnum d-flex">
        ...
        <div class="flex-grow-1">
            <div class="dropdown">
                <button class="w-100 colno dropdown-toggle" type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                Dropdown button
                </button>
                <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                    <a class="dropdown-item" href="#" onclick="$(this).parents('table').data().sheet.sort_col($(this).parents('th').attr('data-col'),'asc')" >&#x1F53C; Sort asc</a>
                    <a class="dropdown-item" href="#" onclick="$(this).parents('table').data().sheet.sort_col($(this).parents('th').attr('data-col'),'desc')">&#x1F53D; Sort desc</a>
                    <a class="dropdown-item" href="#">Reorder columns</a>
                </div>
            </div>
            </div>
        <button class="badge badge-light" onclick="move_col($(this).parents('th').attr('data-col'),'right')">&gt;</button>
    </div>
    </div>
...
<!--    <div><button onclick="tranform(this)">Transform</button></div>-->
    ...
    reorder() {
        let cols = $("<ol type='1' start='0' class='cols-sort'>").sortable();
        this.#fields.forEach((fld,idx)=>{
            $("<li data-col='"+idx+"'>").text(fld).appendTo(cols)
        });
        let thead = this.#thead;
        let trow = thead.children()[0];
        let rows = this.#rows;

        function save_order(cols) {
            let newOrder = cols.find("li").toArray().map(col=>$(col).attr("data-col"));
            console.log(newOrder);
            let headerCols = thead.find("th[data-col]").remove();
            newOrder.forEach((colNo,idx)=>{
                let th = $(headerCols[colNo]).appendTo(trow);
                th.attr("data-col".idx);
            });
        }

        dragable_modal({
            title: "Reorder columns",
            body:$("<div style='max-height: 500px; overflow: auto'>").append(cols),
            buttons:[
                {
                    text: "Save",
                    action: ()=>{save_order(cols)},
                    class: "primary"
                }
            ]
        })

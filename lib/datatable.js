...
<th>
    ...
    lookup2(terms,valueCol,regexp=false,defaultOnEmpty=null){
        ...
        if(selCells.length===1) {
            // console.log("return single record");
            return selCells[0];
        }
        if(selCells.length==0 && defaultOnEmpty!==null){
            // console.log("return defaultOnEmpty");
            return defaultOnEmpty;
        }
        // console.log("return result set");
        return selCells;
    }
    ...
    col_by_name(fld,filter){
        return this.get_col_by_fld(fld,filter);
    }
    col(idf) {
        if(typeof idf==="number") {
            return this.get_col_by_num(fld,filter);
        }
        else {
            return this.get_col_by_fld(fld,filter);
        }

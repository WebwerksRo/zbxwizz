...
class Row {
    ...
    #rowMenu = `<a class="dropdown-item" role='button'  href='#' onclick="$(this).parents('tr').data().rowRef.info()">Row info</a>`+
        ...
        '<a class="dropdown-item" role="button"  href="#" onclick="$(this).parents(\'tr\').data().rowRef.delete()">Delete row</a>' +
        '<a class="dropdown-item" role="button"  href="#" onclick="$(this).parents(\'tr\').data().rowRef.insert(0)">Insert empty row before</a>'+
        '<a class="dropdown-item" role="button"  href="#" onclick="$(this).parents(\'tr\').data().rowRef.insert(1)">Insert empty row after</a>'
        ;

    #btnCellTpl = `<button class="dropdown-toggle w-100" href="#" role="button" data-toggle="dropdown" aria-expanded="false">Tools</button><div class="dropdown-menu dropdown"></div>`;
    hide(){
    ...
    insert(add) {
        this.#table.insert_row(this.#rowIdx+add);
        save_session(true);
    }
    duplicate() {
...
<th>
    <div class="text-center">
        <button onclick="$(this).parents('table').data().sheet.delete_selected()">Del Sel</button>
    </div>
</th>
    `;
    #headerCell = `
<th>
    <div class="colnum">
        <div class="dropdown">
            ...
            <div class="dropdown-menu">
                  <form class="p-2 m-2 border bg-light">
                ...
                <a class="dropdown-item" href='#'>
                    <i class="fa fa-arrows-h"></i> Resize column <input size="3" type="number" name="colWidth" onkeyup="$(this).parents('th').css('width',$(this).val()+'px')">
                </a>
            </div>
        </div>
    </div>
    ...
    show_transform(pos) {
        const transformForm = $("#transformForm").clone().data('sheet',this);
        transformForm.find("input[name=col]").val(pos);

        dragable_modal({
            body: transformForm,
            title: "Transform column "+pos,
            attrs:{
                style: "width: 600px;"
            }
        });
    ...
    rename_fld_dialog(src) {
        ...
        let inp = $("<input style='width: 100%' onkeypress='$(this).removeClass(\"bg-danger\")'>").val(oldName).on("keyup",event=>{
